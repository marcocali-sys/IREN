function BATCH_GUI_PC1_PC2_LOD_VIEWER_SIMPLE()
...
%
% Requisiti:
%   Model .mat: mu, sigma, coeff, predictors
%   Baseline stats CSV: PC1_mean_of_medians, PC1_std_of_medians,
%                       PC2_mean_of_medians, PC2_std_of_medians
%   Monitoring CSV: timestamp + predictors
...
% Right panel to host 2 axes stacked
...
% --- helper: trova colonna che inizia con un certo prefisso ---
...
if any(isempty([i_pc1_mean, i_pc1_std, i_pc2_mean, i_pc2_std]))
    error("Baseline stats CSV must contain columns starting with: " + ...
          "PC1_mean_of*, PC1_std_of*, PC2_mean_of*, PC2_std_of*");
end

% --- estrai valori (prima riga) ---
...
% --- sanity check ---
if any(~isfinite([pc1_mean, pc1_std, pc2_mean, pc2_std])) || ...
   any([pc1_std, pc2_std] <= 0)
    error("Baseline stats contain invalid values (NaN/Inf or non-positive std).");
end

            % PC1
            app.baseline_mean_PC1 = B.PC1_mean_of_medians(1);
            pc1_std = B.PC1_std_of_medians(1);
            app.LOD_lower_PC1 = app.baseline_mean_PC1 - 3*pc1_std;
            app.LOD_upper_PC1 = app.baseline_mean_PC1 + 3*pc1_std;

            % PC2
            app.baseline_mean_PC2 = B.PC2_mean_of_medians(1);
            pc2_std = B.PC2_std_of_medians(1);
            app.LOD_lower_PC2 = app.baseline_mean_PC2 - 3*pc2_std;
            app.LOD_upper_PC2 = app.baseline_mean_PC2 + 3*pc2_std;
